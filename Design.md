...
- **Design**: **Relational + vector + graph**. **Cognee (default)**: SQLite + ChromaDB + Kuzu by default; supports Postgres, Qdrant/LanceDB/PGVector, Neo4j via Cognee `.env`. **In-house (chroma)**: SQLite + Chroma + optional Kuzu/Neo4j via `core.yml`; enterprise: Postgres, Qdrant, etc. See **docs/MemoryAndDatabase.md** (detailed support matrix), **docs/MemoryUpgradePlan.md**, **docs/MemoryCogneeEvaluation.md**.
