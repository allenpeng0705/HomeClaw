...
- **소셜 네트워킹** — **친구**(HomeClaw, Note, Reminder 등 AI 친구 + **사용자형 친구**= 다른 사용자)로 채팅·메모리; **사용자 간 메시지**는 동반 앱에서만 주고받음(받은편지함·푸시). Core는 전달만 하며 LLM 미사용. [SocialNetworkDesign.md](docs_design/SocialNetworkDesign.md) 참조.
...
2. [HomeClaw로 할 수 있는 것](#2-homeclaw로-할-수-있는-것) — 채널, 다중 사용자, [소셜 네트워킹(Companion + Core)](#소셜-네트워킹companion--core)
...
### 소셜 네트워킹 (Companion + Core)

HomeClaw는 단일 인스턴스에서 **자신만의 소셜 네트워크** 허브로 동작합니다(나중에 다중 인스턴스 연결도 설계됨). 소셜 네트워크는 **동반 앱과 Core 전용**이며, 채널(Telegram, Slack 등)은 사용자↔AI 대화용이고 사용자 간 메시지는 포함하지 않습니다.

| 기능 | 설명 |
|------|------|
| **친구 목록** | 각 사용자는 `config/user.yml`에 **friends** 목록을 가집니다: **AI 친구**(HomeClaw, Note, Reminder, 사용자 정의 페르소나)로 채팅·메모리, **사용자형 친구**(같은 HomeClaw의 다른 사용자). |
| **AI 친구와 채팅** | 동반 앱 또는 WebChat에서 대화할 친구를 선택. 각 AI 친구는 `homeclaw_root/{user_id}/{friend_id}/` 아래 자체 identity·knowledge 폴더를 가질 수 있습니다. |
| **사용자 간 메시지** | 사용자는 **동반 앱**을 통해서만 서로 메시지를 보내고 받습니다. Core는 전달만 하며(LLM 미사용). 받은편지함: **GET /api/user-inbox**. 보내기: **POST /api/user-message**. 텍스트·이미지·푸시토크(음성) 지원. |
| **다중 사용자, 단일 Core** | `config/user.yml` 또는 Portal에서 사용자 추가. 각 사용자는 격리된 컨텍스트·샌드박스·친구를 가집니다. 동반 앱은 선택적으로 로그인(사용자명/비밀번호). |

자세한 내용은 [SocialNetworkDesign.md](docs_design/SocialNetworkDesign.md)·[UserToUserMessagingViaCompanion.md](docs_design/UserToUserMessagingViaCompanion.md)·[docs/friends-folders-and-users.md](docs/friends-folders-and-users.md)를 참조하세요.
