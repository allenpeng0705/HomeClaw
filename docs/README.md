...
| **Peer tool (`peer_call`)** | One Core’s LLM can call another Core’s `/inbound` (automation), separate from Companion federation. **Off by default** (`peer_call_enabled: false`); set **`peer_call_enabled: true`** only if you need it. Remote friends use **`federation_enabled`**. | [multi-instance-peers.md](multi-instance-peers.md), [homeclaw-to-homeclaw-and-remote-friends.md](homeclaw-to-homeclaw-and-remote-friends.md) |
