...
| **Identity** | The app sends **user_id** (default `companion`) and optionally **app_id** (agent; omitted → Core uses `homeclaw`). **channel_name: "companion"**. That user must be in **config/user.yml**. Memory/Cognee is scoped by user_id + app_id (e.g. dataset `memory_companion_homeclaw`). Pass different user_id/app_id (e.g. from settings) for per-user or per-agent memory. | The channel sends a **user_id** from the platform (e.g. `telegram_123`, `matrix:@user:domain`) and **channel_name** (e.g. `telegram`, `discord`). Those identities must be listed in **config/user.yml** under the right user’s `im` / `email` / `phone`. |
