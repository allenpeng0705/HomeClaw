...
- **`llama_cpp`** — Defaults for **local** llama.cpp servers: `ctx_size`, `predict`, `temp`, `threads`, `n_gpu_layers`, `repeat_penalty`, `function_calling`, `stop`. **`reasoning_budget`**: set to `0` to disable thinking mode (Qwen: no `<think>` blocks). **`enable_reasoning`**: `false` is an alternative to `reasoning_budget: 0`. **Qwen / Qwen 3.5 (single entry point):** set **`qwen_mode`** to `"qwen3"` or `"qwen35"` here only; optional **`qwen35_grammar_path`** (default: `config/grammars/qwen35_tools.gbnf`), **`qwen35_use_grammar`** (default true). Do not set Qwen options in `skills_and_plugins.yml` or under `completion`. Sub-key **`embedding`** for embedding-model-only overrides.
- **`completion`** — Parameters sent with **every** chat request: `max_tokens`, `temperature`, `top_p`, `repeat_penalty`, `stop`, `image_max_dimension`. **`tool_temperature`**: when set, used only when the request includes tools. **`disable_thinking_when_tools`**: when `true`, sends `enable_thinking: false` to local/Ollama when tools are present. Do **not** set `qwen_model` or `qwen35_*` here; use `llama_cpp` in `llm.yml` for all Qwen settings.

**Qwen (single entry point — `llm.yml` → `llama_cpp` only):**

- **`qwen_mode`**: `"qwen3"` or `"qwen35"` (omit = not Qwen). **qwen3** = 8B: `--reasoning-budget 0`, tool temp 0.1, `<tool_call>` format + NO_TOOL_REQUIRED in system prompt. **qwen35** = 9B (llama.cpp b8150+): same plus presence_penalty 1.5, stop `</think>` and `</tool_call>`, GBNF grammar (path: **`qwen35_grammar_path`**, default `config/grammars/qwen35_tools.gbnf`; disable with **`qwen35_use_grammar: false`**), and **&lt;tools&gt;…&lt;/tools&gt;** block.
