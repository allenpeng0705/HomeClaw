...
| **folder_list** | List one level of a directory (subfolders and files). Use when the user asks for **folder structure** or **what folders/directories exist** (e.g. 'what folders are in my sandbox'). When the user asks for a **named folder** (e.g. 'what files in documents folder', 'list videos'): pass path='documents' (or the folder they said — see `base/user_sandbox_folders.py`: documents, downloads, images, output, work, knowledge, videos, audios, plus **share** for global share) — do NOT omit path or use '.' or you will list the sandbox root. When path not given: lists user sandbox root. Per friend: {FriendName}/. Use path 'share' for global share. Returned 'path' is the exact path to pass to document_read/file_read. |
