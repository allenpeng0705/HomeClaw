...
我决定**向 OpenClaw 学习**，但不丢掉 GPT4People 里已经好用的部分。OpenClaw 有丰富的**技能**生态（SKILL.md、工作流、ClawHub）。于是我让 HomeClaw **支持同一套技能格式**：你可以把 OpenClaw 的技能放到 `skills/` 下，助手就能用。我们保留自己的优势：**本地优先**、**RAG 记忆**、**多用户沙箱**，以及已经做好的**插件模型**。结果不是“克隆 OpenClaw”，而是“取两家之长”。
...
- **OpenClaw 技能：** 可以复用 **OpenClaw 的技能**（`skills/` 下的 SKILL.md）。助手看到“可用技能”并用工具执行这些工作流。不必全部移植——只要能表达成“按这种方式用这些工具”，就可以在 HomeClaw 上跑。
