...
2. **Implement a subclass of BaseChannel** in `channels/<YourChannel>/channel.py`:
   ...
   - **Receive from platform**: Your code (polling, webhook, or client events) receives a user message; build a **PromptRequest** (see `base/base.py`: `request_id`, `channel_name`, `channelType`, `user_name`, `app_id`, `user_id`, `contentType`, `text`, `action`, `host`, `port`, `images`/`videos`/`audios`/`files`, `timestamp`, etc.). For **multi-modal** (image, audio, video): set **images** / **audios** / **videos** (paths or data URLs) and **contentType**; see **docs_design/Multimodal.md** §1. For **files** (any type: image, audio, video, PDF, Word, Excel): save each file to disk in **your channel’s own doc folder** (e.g. `channels/<channel_name>/docs/`) and set **request.files = [path1, path2, ...]**; Core runs **file-understanding** (detects type, handles media, extracts text from documents and injects into the message). The **knowledge base** is **user-based** (per user_id), not per-channel. See **docs_design/FileUnderstandingDesign.md** (§6 doc folder, §7 KB) and **Multimodal.md** §1b.
   - **Send to Core**:
