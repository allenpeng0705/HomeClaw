...
| **Order** | **RAG “RelevantMemory”** order is by **relevance** (highest score first) by default. Set **memory_context_order: newest_first** in config (core.yml or memory_kb.yml) to sort by **created_at** descending so “1” is the most recent memory when the backend returns timestamps. **Chat history** (conversation turns passed to the LLM) is always **chronological** (oldest first): built from `chatDB.get()` which returns rows by `created_at` desc then reversed, so the model sees user/assistant in time order. | `core/llm_loop.py` (memory sort); `memory/chat/chat.py` (`get` order_by desc + reversed). |
