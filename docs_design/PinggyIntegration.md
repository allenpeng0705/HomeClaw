...
**Alternative:** If you use **Cloudflare Tunnel**, **Tailscale Funnel**, or any other service and already have a public URL, set **`core_public_url`** in core.yml to that URL. **GET /pinggy** will then show that URL and a QR code for Companion scan-to-connect; no Pinggy tunnel is started. See [Remote access](docs/remote-access.md).
