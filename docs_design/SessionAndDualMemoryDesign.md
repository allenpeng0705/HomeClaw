...
- **Skills/plugins selection:** Skills: `skills_use_vector_search=false` → include all skills; `true` → RAG (skills_max_retrieved → threshold → skills_max_in_prompt). Plugins: same — `plugins_use_vector_search=false` → include all; `true` → RAG (plugins_max_retrieved → threshold → plugins_max_in_prompt). See `config/core.yml`.
