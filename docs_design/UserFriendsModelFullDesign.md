...
- **Each friend** in `friends` is an object:
  ...
  - **type** (optional, future) — `core_role` (default) = AI companion; `user` = this friend is another user in the same HomeClaw. When `type: user`, **user_id** (required) is the system user id of that user; Core forwards messages to them without calling the LLM. See [UserToUserMessagingViaCompanion.md](UserToUserMessagingViaCompanion.md).
