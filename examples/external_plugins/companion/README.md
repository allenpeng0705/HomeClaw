...
Edit **`examples/external_plugins/companion/config.yml`**:
...
```bash
COMPANION_PORT=3104 python -m examples.external_plugins.companion.server
```
...
```bash
CORE_URL=http://your-core:9000 CORE_API_KEY=your-key python -m examples.external_plugins.companion.register
```
...
| **Companion server not reachable** | Ensure the server is running (`python -m examples.external_plugins.companion.server`) and that `COMPANION_PORT` (default 3103) is not in use by another process. Core must be able to reach the plugin’s base_url (e.g. `http://127.0.0.1:3103`). |
