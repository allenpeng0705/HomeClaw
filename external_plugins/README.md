...
**Summary:** Small examples in this folder — **Python** (Time, Friends), **Node.js** (Quote), **Go** (Time), **Java** (Quote). Full-featured examples: **system_plugins/homeclaw-browser** (Node.js — WebChat, browser automation, canvas, nodes) and the **Friends plugin** (Python — persona/companion; see repo for location).
...
- **All plugins:** Starts time (3102), friends (3103), quote-node (3111), time-go (3112), quote-java (3113) in the background and registers each with Core.
- **Specific plugins:** `./external_plugins/run.sh time friends quote-node` or `.\external_plugins\run.ps1 -Plugins time,friends,quote-node`
...
./external_plugins/friends/run.sh
...
.\external_plugins\friends\run.ps1
...
Then you can ask: "What time is it in Tokyo?", "Give me a quote", or use the Friends persona (enable in `config/core.yml` as below).
...
- **Server**: Separate conversation thread (persona: girlfriend/boyfriend/friend etc.) per user. Chat stored **only in friends store** (`database/friends_store`), not in main user DB. Uses Core's LLM via `POST /api/plugins/llm/generate`. See [docs_design/CompanionFeatureDesign.md](../docs_design/CompanionFeatureDesign.md).
- **Port**: 3103 (set `FRIENDS_PORT` to change)
- **Capability**: **chat** — one turn with the persona; user message in `user_input`, returns persona reply.

### Run

**One command (start + register):** From project root after Core is running: `./external_plugins/friends/run.sh` or `.\external_plugins\friends\run.ps1`

Or manually: start server (`python -m external_plugins.friends.server`), then register (`python -m external_plugins.friends.register`).
...
Then: **Companion app**, **WebChat**, or **homeclaw-browser** control UI send `conversation_type: companion` or `session_id: companion` (or `channel_name: companion`); Core routes those requests to the Friends plugin only.
...
## 7. Friends plugin (Python)

The **Friends** plugin (Python) provides a persona (girlfriend/boyfriend/friend etc.) with its own chat store. Same contract: HTTP server with GET /health and POST /run; register with Core via POST /api/plugins/register. See **external_plugins/friends/** for run steps and README.
