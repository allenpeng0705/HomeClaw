...
- **feishu-cli-1.0.0** — Feishu / Lark via official **`lark-cli`**. If the binary is missing, the runner runs **`npx --yes @larksuite/cli@latest install`** once (needs Node + `npx`); set **`LARK_CLI_AUTO_INSTALL=0`** to skip. Still run **`config init`** / optional **`auth login`** on the Core host; see [Feishu CLI doc](https://open.feishu.cn/document/mcp_open_tools/feishu-cli-let-ai-actually-do-your-work-in-feishu). **run_skill**(skill_name=`feishu-cli-1.0.0`, script=`lark_cli_runner.py`, args=[`discover`] or [`exec`, …]). Env: **`LARK_CLI`**, **`LARK_CLI_TIMEOUT_SEC`**, **`LARK_CLI_INSTALL_TIMEOUT_SEC`**.
