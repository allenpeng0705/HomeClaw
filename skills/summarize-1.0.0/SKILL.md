...
## Long summaries -> VMPrint preview first

When the summary is **long** (multiple sections, several paragraphs, or clearly structured in Markdown):

1. **Always** return the summary in your reply so the user sees it in chat.
2. **If `vmprint_render` is available:** Call it with the summary Markdown and
   - `path=output/summary_<slug>.preview.html`
   - `output_format=browser_preview_html`
   - `vmprint_profile=literature`
   Return the preview link in your reply.
3. Generate PDF only when user explicitly asks for print/download/export (use `vmprint_render` `output_format=pdf`, or `markdown_to_pdf` as fallback).
4. If VMPrint tools are unavailable, save Markdown via **file_write** to **output/summary_<slug>.md** and return that link.

The user does not have to say "PDF" or "markdown"; for long summaries, automatically offer preview link first and PDF on explicit request.
