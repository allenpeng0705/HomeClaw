...
async def test_llm_connection():
    ...
    Uses litellm.acompletion directly instead of LLMGateway.acreate_structured_output
    to avoid instructor's sync retry path (retry_sync) being used with an async client,
    which causes TypeError: 'coroutine' object is not callable.
    """
    try:
        logger.info("Testing connection to LLM endpoint...")
        try:
            client = get_llm_client(raise_api_key_error=False)
        except Exception as e:
            raise RuntimeError(
                f"LLM client could not be initialized: {e!s}. "
                "Check LLM provider and config (e.g. API key, model)."
            ) from e
        if not getattr(client, "model", None):
            raise RuntimeError(
                "LLM client has no model configured. Check LLM_MODEL (or equivalent) config."
            )
        response = await asyncio.wait_for(
            litellm.acompletion(
                ...
                api_base=getattr(client, "endpoint", None),
